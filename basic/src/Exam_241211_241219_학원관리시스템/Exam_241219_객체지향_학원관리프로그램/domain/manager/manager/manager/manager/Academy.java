...
package Exam_241219_객체지향_학원관리프로그램.domain.manager.manager.manager.manager;
...
//        }
        ...
        if (isStudentCheck && isCourseCheck) {
            isTaskSuccess = true;
        }
// 3. 학생의 수강 가능 여부 검사
// - 학생의 현재 수강중인 강좌 수 확인
        ...
        for (Student student : studentsList) {
            for (Course course : student.getCurrentCourseList()) {
                if (course != null && course.getCourseName().equals(courseName)) {
                    // 여기서 중복 수강 확인 후 처리가 없음
                    System.out.println("이미 수강중인 강좌입니다.");
                    isTaskSuccess = false;
                    return isTaskSuccess;
                }
            }
...
// 현재: 코스 리스트의 null을 찾아서 처리
// 힌트: 학생을 먼저 찾고, 그 학생의 수강 목록에서 빈자리를 찾아야 합니다
        // 1. 먼저 찾은 강좌를 저장해둘 변수 필요
        Course targetCourse = null;

        for (Student student : studentsList) {
            if (student.getStudentId().equals(studentId)) {
//            학생의 수강 목록에서 빈자리 찾기
                Course[] courseList = student.getCurrentCourseList();
                for (int i = 0; i < courseList.length; i++) {
                    if (courseList[i] == null) {
//                    여기가 빈자리
//                    새로운 강좌를 만드는 게 아니라
//기존 강좌 목록(courseList)에서 해당 강좌를 찾아서
//학생의 수강 목록에 추가해야 함
//                    courseList[i] = newCourse(courseName,0); // 값을 어떻게 줘야하지..
//                    courseList[i] = courseList[i].getCourseId().equals(courseId);
                        // 2. 처음 강좌 확인할 때 찾은 강좌 저장
                        for (Course course : courseList) {
                            if (course.getCourseId().equals(courseId)) {
                                targetCourse = course;
                                break;
                        ...
                        // 3. 나중에 수강신청 처리할 때 저장된 강좌 사용
// (위의 수강신청 처리 부분에서)
                        courseList[i] = targetCourse;
                    }
                }
            }
        }
...
// - 수강신청 성공/실패 메시지 출력
        if (isTaskSuccess) {
            ...
            System.out.println("수강 신청이 실패하였습니다.");
        }
        return isTaskSuccess;
    }
